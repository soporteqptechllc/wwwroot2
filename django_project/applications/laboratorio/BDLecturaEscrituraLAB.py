...
try:
    ...
    # **********************************************************************************      
    # **********************************************************************************
    #print("Inicio de la carga en tabla de las listas del 0 al BM")
    #print(coluBM)   
    ...
    cnxn3.commit() # Confirmacion del Qury
    #print("Fin de la carga en tabla de las listas del 0 al BM")
    # **********************************************************************************
    # **********************************************************************************
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            # Declaro variable
            datofecha = f
            datovalor = (float(v)+float(ctesuma))
            # Crear el texto del Query
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=14 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0
        else:
            ctesuma += v
    # ---------------------------------------------------------------------------------
    # Declaro variable de id de cemento
    txtcemento = 'Concretero'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_P_V 
    WHERE PRODUCTO = '%s' AND COMPONENTE LIKE \'Producción%%\'
    GROUP BY FECHA
    ORDER BY FECHA""" % (txtcemento)   
    # Ejecutar una consulta
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=7 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0
    ...
    # **********************************************************************************
    # Declaro variable de id de cemento
    txtcemento = 'Uso General 42.5 KG'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_E_V 
    WHERE PRODUCTO = '%s' AND COMPONENTE LIKE \'Despacho%%\'
    GROUP BY FECHA
    ORDER BY FECHA""" % (txtcemento)   
    # Ejecutar una consulta
    cursor2.execute(texto_query)   
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=9 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=2 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0
    ...
    # **********************************************************************************
    # Declaro variable de id de cemento
    txtcemento = 'Estructutal 42.5 KG'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_E_V 
    WHERE PRODUCTO = '%s' AND COMPONENTE = 'Despacho Granel'
    GROUP BY FECHA
    ORDER BY FECHA""" % (txtcemento)
    # Ejecutar una consulta
    cursor2.execute(texto_query)   
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=8 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=1 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_E_V 
    WHERE PRODUCTO = '%s' AND COMPONENTE LIKE 'Despacho Empacado'
    GROUP BY FECHA
    ORDER BY FECHA""" % (txtcemento)
    # Ejecutar una consulta
    cursor2.execute(texto_query)  
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=29 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    # ********************************************************************************** 
    # Declaro variable de id de cemento
    txtcemento = 'Puzolanico 42.5 KG' # O 'Puzolana Molida'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_E_V 
    WHERE PRODUCTO LIKE \'Puzola%%\' AND COMPONENTE LIKE \'Despacho%%\'
    GROUP BY FECHA
    ORDER BY FECHA"""   
    # Ejecutar una consulta
    cursor2.execute(texto_query)   
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=11 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    # ---------------------------------------------------------------------------------
    # Declaro variable de id de cemento
    txtcemento = 'Puzolana Molida'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_P_V 
    WHERE PRODUCTO LIKE \'Puzolana%%\' AND COMPONENTE LIKE \'Producción%%\'
    GROUP BY FECHA
    ORDER BY FECHA"""   
    # Ejecutar una consulta
    cursor2.execute(texto_query)   
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=4 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=10 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=3 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=12 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    # **********************************************************************************
    # Declaro variable de id de cemento
    txtcemento = 'Albañilería 42.5 KG'
    # Crear el texto del Query
    texto_query = """SELECT FECHA, SUM(VALOR) 
    FROM Excel2_E_V 
    WHERE PRODUCTO = '%s' AND COMPONENTE LIKE \'Despacho%%\'
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=32 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
    ...
    for f,v in cursor2:
        ...
        if f in colufecha:
            ...
            texto_query2 ="""UPDATE public.laboratorio_reportelab
            SET toneladas=%s
            WHERE productoid=31 AND datetime=\'%s\' """ % (datovalor,datofecha)
            # Ejecutar una consulta
            cursor3.execute(texto_query2)
            cnxn3.commit() # Confirmacion del Qury
            ctesuma = 0 
            ...
            ctesuma += v    
    # **********************************************************************************
    #print("Fin del calculo de toneladas")           
 # ****************************** FIN **********************************************
